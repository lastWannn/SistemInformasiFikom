    ...
    <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="mb-8">
            <div class="flex items-center gap-3 mb-2">
                <a href="<?= url('/koordinator/problems/' . $problem['id']) ?>" class="text-slate-400 hover:text-slate-600 transition-colors">
                    <i class="bi bi-arrow-left text-xl"></i>
                </a>
                <h1 class="text-3xl font-bold text-slate-800">Edit Laporan #<?= $problem['id'] ?></h1>
            </div>
            <p class="text-slate-500">Ubah informasi laporan permasalahan laboratorium.</p>
        </div>
        ...
        <div class="bg-white rounded-xl shadow-sm border border-slate-200 p-8">
            <form method="POST" action="<?= url('/koordinator/problems/' . $problem['id'] . '/edit') ?>" class="space-y-6">
                ...
                <div>
                    ...
                    <input type="text" id="pc_number" name="pc_number" value="<?= htmlspecialchars($problem['pc_number'] ?? '') ?>" placeholder="Contoh: 01, 02, Server, dll." class="block w-full px-4 py-3 border border-slate-300 rounded-lg focus:ring-2 focus:ring-sky-500 focus:border-sky-500">
                </div>
                ...
                <div>
                    ...
                    <textarea id="description" name="description" rows="5" required placeholder="Jelaskan masalah secara detail..." class="block w-full px-4 py-3 border border-slate-300 rounded-lg focus:ring-2 focus:ring-sky-500 focus:border-sky-500"><?= htmlspecialchars($problem['description']) ?></textarea>
                </div>
                ...
                <div>
                    ...
                    <select id="status" name="status" required class="block w-full px-4 py-3 border border-slate-300 rounded-lg focus:ring-2 focus:ring-sky-500 focus:border-sky-500">
                        <option value="reported" <?= $problem['status'] == 'reported' ? 'selected' : '' ?>>Pending</option>
                        <option value="in_progress" <?= $problem['status'] == 'in_progress' ? 'selected' : '' ?>>Diproses</option>
                        <option value="resolved" <?= $problem['status'] == 'resolved' ? 'selected' : '' ?>>Selesai</option>
                    </select>
                </div>

                <!-- Action Buttons -->
                <div class="flex gap-3 pt-4 border-t border-slate-200">
                    <a href="<?= url('/koordinator/problems/' . $problem['id']) ?>" class="flex-1 px-6 py-3 bg-slate-100 hover:bg-slate-200 text-slate-700 font-medium rounded-lg transition-colors text-center">
                        Batal
                    </a>
