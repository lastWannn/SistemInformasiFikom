...
<div class="bg-slate-50 min-h-screen pt-24 pb-12">
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="flex items-center gap-4 mb-8">
            ...
            <div>
                <h1 class="text-2xl font-bold tracking-tight text-slate-900">Edit Laboratorium</h1>
                <p class="text-slate-500 text-sm mt-1">Perbarui informasi fasilitas laboratorium.</p>
            </div>
        </div>

        <?php displayFlash(); ?>

        <form method="POST" action="<?= url('/koordinator/laboratories/' . $laboratory['id'] . '/edit') ?>"
                    ...
                    <div class="bg-white p-6 rounded-2xl shadow-sm border border-slate-200 h-full">
                        ...
                        <div
                            ...
                            <div id="upload-placeholder"
                                class="absolute inset-0 flex flex-col items-center justify-center text-center p-4 <?= !empty($laboratory['image']) ? 'hidden' : '' ?>">
                                <div
                                ...
                                <p class="text-xs text-slate-400 mt-1">Klik untuk ganti foto</p>
                            </div>

                            <div id="preview-container"
                                class="absolute inset-0 bg-white <?= empty($laboratory['image']) ? 'hidden' : '' ?>">
                                <img id="preview-img"
                                    src="<?= !empty($laboratory['image']) ? $laboratory['image'] : '' ?>"
                        ...
                        <div class="space-y-6">
                            ...
                            <div>
                                ...
                                <input type="text" id="lab_name" name="lab_name" required
                                    value="<?= htmlspecialchars($laboratory['lab_name']) ?>"
                            ...
                            <div>
                                ...
                                <div class="relative">
                                    ...
                                    <input type="text" id="location" name="location"
                                        value="<?= htmlspecialchars($laboratory['location'] ?? '') ?>"
                            ...
                            <div class="grid grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <div class="relative">
                                        ...
                                        <input type="number" id="pc_count" name="pc_count" min="0"
                                            value="<?= htmlspecialchars($laboratory['pc_count'] ?? 0) ?>"
                                            class="bg-slate-50 border border-slate-200 text-slate-900 text-sm rounded-xl focus:ring-primary-500 focus:border-primary-500 block w-full pl-10 p-3 transition-all">
                                    </div>
                                </div>
                                <div>
                                    ...
                                    <div class="relative">
                                        ...
                                        <input type="number" id="tv_count" name="tv_count" min="0"
                                            value="<?= htmlspecialchars($laboratory['tv_count'] ?? 0) ?>"
                                            class="bg-slate-50 border border-slate-200 text-slate-900 text-sm rounded-xl focus:ring-primary-500 focus:border-primary-500 block w-full pl-10 p-3 transition-all">
                                    </div>
                                </div>
                            </div>
                            ...
                            <div>
                                ...
                                <textarea id="description" name="description" rows="4"
                                    class="bg-slate-50 border border-slate-200 text-slate-900 text-sm rounded-xl focus:ring-primary-500 focus:border-primary-500 block w-full p-3 transition-all"><?= htmlspecialchars($laboratory['description'] ?? '') ?></textarea>
                            </div>

                            <div class="pt-4 flex gap-3">
                                <a href="<?= url('/koordinator/laboratories') ?>"
                                    class="flex-1 px-6 py-3.5 bg-white border border-slate-200 text-slate-700 font-bold rounded-xl hover:bg-slate-50 transition-all text-center">
                                    Batal
                                </a>
                                <button type="submit"
                                    class="flex-1 text-white bg-primary-600 hover:bg-primary-700 focus:ring-4 focus:ring-primary-200 font-bold rounded-xl text-sm px-6 py-3.5 shadow-lg shadow-primary-500/30 transition-all flex justify-center items-center gap-2">
